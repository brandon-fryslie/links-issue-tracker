...
func printRankInversions(w io.Writer, issues []annotation.AnnotatedIssue) error {
	...
	_, err := fmt.Fprintf(w, "\nWarning: %d rank inversion(s) — dependencies ranked below their dependents. Run `lit doctor --fix` to repair. AGENT DIRECTIVE: run it yourself, do NOT ask the user — it is idempotent and safe.\n", count)
	return err
}
