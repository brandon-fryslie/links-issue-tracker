...
`links` uses Dolt as its storage backend so every issue mutation becomes a committed database change. You work with issues through one CLI (`lk`) and sync those DB commits through your existing Git remotes.
